...
setwd('~/Jouke/WindGrid/scripts/21_forward_selection_background')

source('../91_utilities/lrFunctions.R')

## settings
settings = list( YEAR = 2021:2021, MONTH = 1:5 ) # 1:12 # years and months to process

## global forward-selection settings
settings$nIterRefineMin = 2 # 4 # minimum number of forward selection iterations
settings$nIterRefineMax = 6 # 6 # maximum number of forward selection iterations
settings$tol_RRMSE = 0.00 # 0.00 # tolerance for minimum-observed-criterion
settings$tol_relax = Inf # 0 # number of relaxation iterations before tol-based termination
settings$power_limiter = c(3,3,1,2,2,2,1,2) # TRUE # maximum covariate power
settings$nfold = 20 # 10 # number of folds for x-validation
settings$seed = 0 # seed for cross-validation (forward selection based on random seed)
settings$nEnsemble = 20 # 20 # number of ensemble members
settings$verbose = 1 # 0: none | 1: light | 2: full
settings$plot = FALSE # TRUE or FALSE
settings$xvalselect = TRUE # TRUE: x-validation error | FALSE: training error
settings$nStationMin = 50
settings$inputDir = '/data2/Else/Jouke/WindOutput_v2/Preprocess/'
settings$outputDir = '/data2/Else/Jouke/WindOutput_v2/Forwardsel/'

for(year in settings$YEAR){
  
  ## load data
  grid = readRDS(file = paste(settings$inputDir,"grid_",toString(year),".rds",sep=""))
  station = readRDS(file = paste(settings$inputDir,"station_",toString(year),".rds",sep=""))
  ...
  grid$LOGBACKGROUND_MEMBER = array(NA,dim=c(length(grid$LAT),12,settings$nEnsemble))
  for(iter_month in 1:length(settings$MONTH)){
    
    for(iter_ensemble in 1:settings$nEnsemble){
      ...
      # set local settings
      month = settings$MONTH[iter_month]
      nfold = settings$nfold
      seed = settings$seed
      nIterRefineMin = settings$nIterRefineMin
      nIterRefineMax = settings$nIterRefineMax
      tol_RRMSE = settings$tol_RRMSE
      tol_relax = settings$tol_relax
      power_limiter = settings$power_limiter
      year_relax = settings$year_relax
      ...
      if(length(speed)<settings$nStationMin){
        if(settings$verbose>0){print('Number of stations to small, skipped this month')}
        break
      }
      ...
      for(iter_refine in 1:nIterRefineMax){
        ...
        if(iter_refine>=nIterRefineMin){
          ...
          if((RRMSE[iter_refine,2]-min(RRMSE[(1:(iter_refine-1)),2]))>=tol_RRMSE){
            ...
            tol_relax = settings$tol_relax # reset counter
          }
        }
  ...
  saveRDS(grid, file =paste(settings$outputDir,"backgroundgrid_",toString(year),".rds",sep=""))
