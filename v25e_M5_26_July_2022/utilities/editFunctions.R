...
EDITfactorByProxy <- function(EDITproxyStation,EDITproxy,ensemble,coefficient){
  # normalize
  ...
  #print(size(X))
  #print(coefficient)
  factor1 = exp(X%*%t(coefficient))
  factor2 = rowMeans(ensemble) / (rowMeans(ensemble)-rowMins(ensemble)+1e-6)
  factor = factor1
  #factor = pmin(factor1,factor2)
...
EDITcostfunction <- function(pars,EDITproxyStation,ensemble,reference,Exval,minvals,type)
{
  coefficients = t(pars)
  #print(coefficients)
  ...
  # factor ensemble
  ensembleTest = EDITfactorByProxy(EDITproxyStation,EDITproxyStation,ensemble,coefficients)
